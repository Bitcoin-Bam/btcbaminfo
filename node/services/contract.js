...
const totalSupplyABI = Solidity.brc20ABIs.find(abi => abi.name === 'totalSupply')
const balanceOfABI = Solidity.brc20ABIs.find(abi => abi.name === 'balanceOf')
const ownerOfABI = Solidity.brc721ABIs.find(abi => abi.name === 'ownerOf')
...
class ContractService extends Service {
  ...
  async start() {
    ...
    this.#BRC20 = this.node.getModel('brc20')
    this.#BRC20Balance = this.node.getModel('brc20_balance')
    this.#BRC721 = this.node.getModel('brc721')
    this.#BRC721Token = this.node.getModel('brc721_token')
    ...
    for (let transaction of block.transactions) {
      for (let i = 0; i < transaction.outputs.length; ++i) {
        ...
        if (output.scriptPubKey.type === OutputScript.EVM_CONTRACT_CREATE) {
          ...
          if (contract && contract.type === 'brc20') {
  ...
  async _syncContracts() {
    ...
    if (contractsToRemove.length) {
      await this.#db.query(sql`
        DELETE contract, tag, brc20, brc20_balance, brc721, brc721_token
        FROM contract
        LEFT JOIN contract_tag tag ON tag.contract_address = contract.address
        LEFT JOIN brc20 ON brc20.contract_address = contract.address
        LEFT JOIN brc20_balance ON brc20_balance.contract_address = contract.address
        LEFT JOIN brc721 ON brc721.contract_address = contract.address
        LEFT JOIN brc721_token ON brc721_token.contract_address = contract.address
  ...
  async _createContract(address, vm) {
    ...
    if (isBRC721(code)) {
      let [nameResult, symbolResult, totalSupplyResult] = await this._batchCallMethods([
        {address, abi: Solidity.brc721ABIs.find(abi => abi.name === 'name')},
        {address, abi: Solidity.brc721ABIs.find(abi => abi.name === 'symbol')},
        {address, abi: Solidity.brc721ABIs.find(abi => abi.name === 'totalSupply')}
      ])
      try {
        ...
        await contract.save()
        await this.#ContractCode.bulkCreate([{sha256sum, code}], {ignoreDuplicates: true})
        await this.#ContractTag.create({contractAddress: address, tag: 'brc721'})
        await this.#BRC721.create({
    ...
    } else if (isBRC20(code)) {
      let [
        nameResult, symbolResult, decimalsResult, totalSupplyResult, versionResult
      ] = await this._batchCallMethods([
        {address, abi: Solidity.brc20ABIs.find(abi => abi.name === 'name')},
        {address, abi: Solidity.brc20ABIs.find(abi => abi.name === 'symbol')},
        {address, abi: Solidity.brc20ABIs.find(abi => abi.name === 'decimals')},
        {address, abi: Solidity.brc20ABIs.find(abi => abi.name === 'totalSupply')},
        {address, abi: Solidity.brc20ABIs.find(abi => abi.name === 'version')}
      ])
      try {
        let version
        try {
        ...
        await contract.save()
        await this.#ContractCode.bulkCreate([{sha256sum, code}], {ignoreDuplicates: true})
        await this.#ContractTag.create({contractAddress: address, tag: 'brc20'})
        await this.#BRC20.create({
          contractAddress: address,
          name,
          symbol,
          decimals,
  ...
  async _processReceipts(block) {
    ...
    for (let addressString of totalSupplyChanges) {
      ...
      if (contract) {
        ...
        if (contract.type === 'brc20') {
          await this.#BRC20.update({totalSupply}, {where: {contractAddress: address}})
        } else {
          await this.#BRC721.update({totalSupply}, {where: {contractAddress: address}})
        }
      }
    }
  }
  ...
  async _updateBalances(balanceChanges) {
    ...
    if (operations.length) {
      await this.#BRC20Balance.bulkCreate(operations, {updateOnDuplicate: ['balance'], validate: false})
    }
  }
  ...
  async _updateTokenHolders(transfers) {
    ...
    await this.#BRC721Token.bulkCreate(operations, {updateOnDuplicate: ['holder'], validate: false})
  }
}

function isBRC20(code) {
  return code.includes(balanceOfABI.id)
    && code.includes(transferABI.id)
    && code.includes(TransferABI.id)
}

function isBRC721(code) {
  return code.includes(balanceOfABI.id)
